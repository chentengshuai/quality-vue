          ...
          <template v-if="showAll">
            <el-col :span="8">
              <el-form-item label="规格型号">
                <el-input
                  v-model="query.materialSpecs"
                  placeholder="请输入规格型号查询"
                  clearable
                  @keyup.enter.native="search()"
                />
              </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      columnList: [
        ...
        { prop: "materialSpecs", label: "规格" },
        { prop: "noticeNum", label: "单据号" },
        { prop: "inspectorName", label: "检验员" },
        { prop: "detectionTime", label: "检测日期" },
        { prop: "vendorName", label: "供应商" },
        { prop: "materialNumber", label: "数量" },
      ],
    };
  },
  computed: {},
  ...
  methods: {
    ...
    initData() {
      ...
      request({
        url: `/api/project/BizMaterialInspectNotice/materialsNoticeList`,
        method: "post",
        data: _query,
      }).then((res) => {
        var _list = [];
    ...
    reset() {
      this.listQuery.noticeNum = "";
      this.listQuery.inspectorId = "";
      this.listQuery.currentPage = 1;
      this.listQuery.pageSize = this.hasPage ? this.pageSize : 20;
      this.initData();
    },
