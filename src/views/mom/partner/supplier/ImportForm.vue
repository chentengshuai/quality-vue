  ...
  <el-dialog title="批量导入" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <div class="import-main" v-show="active==2">
      <JNPF-table v-loading="listLoading" :data="list">
        <el-table-column prop="partnerCode" label="业务伙伴编码" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.partnerCode"/>
          </template>
        </el-table-column>
    ...
    <div class="import-main" v-show="active==3">
      ...
      <div class="unsuccess" v-if="result.resultType">
        ...
        <JNPF-table :data="resultList" height="280px">
          <el-table-column prop="partnerCode" label="业务伙伴编号" width="100"/>
        </JNPF-table>
      </div>
    </div>
    ...
    methods: {
      ...
      next() {
        if (this.active == 2) {
          ...
          return request({
            url: '/api/project/Partner/ImportData',
            method: 'post',
            data: {
              list: this.list
            }
        ...
        if (this.active == 1) {
          ...
          this.btnLoading = true
          request({
            url: '/api/project/Partner/ImportPreview',
            method: 'get',
      ...
      templateDownload() {
        return request({
          url: '/api/project/Partner/templateDownload',
          method: 'get'
        }).then(res => {
