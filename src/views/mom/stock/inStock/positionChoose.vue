            ...
            <el-form @submit.native.prevent>
              <el-col :span="8">
                <el-form-item label="仓库编码">
                  <el-input v-model="listQuery.warehouseCode" placeholder="请输入仓库编码查询" clearable
                    @keyup.enter.native="search()" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
                <el-form-item label="仓库名称">
                  <el-input v-model="listQuery.warehouseName" placeholder="请输入仓库名称查询" clearable
                    @keyup.enter.native="search()" />
                </el-form-item>
              </el-col>
              <el-col :span="8">
  ...
  export default {
    ...
    methods: {
      ...
      initData() {
        ...
        request({
          url: `/api/project/Warehouse/selectWarehouseList`,
          method: 'post',
          data: _query
        }).then(res => {
          var _list = [];
      ...
      rowClick(row, preRow) {
            this.$emit("returnPositionData", row);
      },
      select() {
            this.$emit('returnPositionData', this.checked)
      },
    }
  }
</script>
