  ...
  <el-dialog title="批量导入" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <div class="import-main" v-show="active==2">
      <JNPF-table v-loading="listLoading" :data="list">
        <el-table-column prop="materialNumber" label="物料编码" width="150">
    ...
    <div class="import-main" v-show="active==3">
      ...
      <div class="unsuccess" v-if="result.resultType">
        ...
        <JNPF-table :data="resultList" height="280px">
          <el-table-column prop="materialNumber" label="物料编号" width="100"/>
        </JNPF-table>
      </div>
    </div>
    ...
    methods: {
      ...
      next() {
        if (this.active == 2) {
          ...
          return request({
            url: '/api/project/Mom_material/ImportData',
            method: 'post',
            data: {
              list: this.list
            }
        ...
        if (this.active == 1) {
          ...
          this.btnLoading = true
          request({
            url: '/api/project/Mom_material/ImportPreview',
            method: 'get',
      ...
      templateDownload() {
        return request({
          url: '/api/project/Mom_material/templateDownload',
          method: 'get'
        }).then(res => {
