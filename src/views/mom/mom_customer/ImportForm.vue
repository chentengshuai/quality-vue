  ...
  <el-dialog title="批量导入" :close-on-click-modal="false" :visible.sync="visible"
    ...
    <div class="import-main" v-show="active==2">
      <JNPF-table v-loading="listLoading" :data="list">
        <el-table-column prop="customerNumber" label="客户编码" width="150">
          <template slot-scope="scope">
            <el-input v-model="scope.row.customerNumber"/>
          </template>
        </el-table-column>
    ...
    <div class="import-main" v-show="active==3">
      ...
      <div class="unsuccess" v-if="result.resultType">
        ...
        <div class="upload error-show">
          ...
          <div class="up_right">
            <p class="tip">正常数量条数：
              <el-link type="success" :underline="false">{{result.snum}}条
              </el-link>
            </p>
            <p class="tip">异常数量条数：
              <el-link type="danger" :underline="false">{{result.fnum}}条
              </el-link>
            </p>
          </div>
        </div>
        ...
        <JNPF-table :data="resultList" height="280px">
          <el-table-column prop="customerNumber" label="物料编号" width="100"/>
        </JNPF-table>
      </div>
    </div>
    ...
    methods: {
      ...
      next() {
        if (this.active == 2) {
          ...
          return request({
            url: '/api/project/Mom_customer/ImportData',
            method: 'post',
            data: {
              list: this.list
            }
        ...
        if (this.active == 1) {
          ...
          this.btnLoading = true
          request({
            url: '/api/project/Mom_customer/ImportPreview',
            method: 'get',
      ...
      templateDownload() {
        return request({
          url: '/api/project/Mom_customer/templateDownload',
          method: 'get'
        }).then(res => {
